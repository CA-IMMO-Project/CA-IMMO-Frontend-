...
#[AsCommand(name: 'app:seed:lands', description: 'Seed the database with the CA IMMO demo land listings')]
class SeedLandsCommand extends Command
{
    public function __construct(
        private readonly EntityManagerInterface $em,
        private readonly LandRepository $lands,
    ) {
        parent::__construct();
    }
    ...
    protected function execute(InputInterface $input, OutputInterface $output): int
    {
        ...
        foreach ($seeds as $seed) {
            $existing = $this->em->getRepository(Land::class)->findOneBy(['title' => $seed['title']]);
            if ($existing) {
                continue;
            }

            $land = new Land();
            $land->setTitle($seed['title']);
            $land->setDescription($seed['description']);
            $land->setPrice($seed['price']);
            $land->setRegion($seed['region']);
            $land->setLocation($seed['location']);
            $land->setLatitude($seed['coordinates'][0]);
            $land->setLongitude($seed['coordinates'][1]);
            $land->setImageUrl($seed['imageUrl']);
            $land->setFeatures($seed['features']);
            $land->setArea($seed['area']);
            $land->setTitleStatus($seed['titleStatus']);
            $land->setStatus($seed['status']);
        ...
        $io->success(sprintf('%d terrain(s) créé(s), %d déjà existant(s).', $created, count($seeds) - $created));
